 ...
 * 
 */
var Rewriter = module.exports = function Rewriter(options,server) {
...
Rewriter.prototype.sync = function(){
    ...
    exec = function(data) {
        self.count+=1;
		    invoke(self.write[data.key],server.client,data.val,function(){
            self.count-=1;
        });
    };
...
Rewriter.prototype.flush = function(key,val){
    ...
    self.count+=1;
		return invoke(self.write[key],server.client,val,function(){self.count-=1;});
};
